...
try {
		...
		$db = new MongoClient($configuration['mongo']['hostname'], array(
				"username" => $configuration['mongo']['username'],
				"password" => $configuration['mongo']['password'],
				"db" => $configuration['mongo']['database']
		));
		...
		$mongoFra = DateTime::createFromFormat('!d/m/Y H:i', '01/01/2013 00:00', new DateTimeZone('UTC'));
		$mongoTil = DateTime::createFromFormat('!d/m/Y H:i', '01/01/2014 00:00', new DateTimeZone('UTC'));
...
//				'sensor' => 'z1t',
		...
		$results = $db->selectDB('seih')->command(
